...
async function fetchOfferViaBrowser(vendor, medicationName, quantity) {
...
async function fetchOfferViaJson(vendor, medicationName, quantity) {
    const url = `${apiBaseUrl()}/dummy/${vendor}?name=${encodeURIComponent(medicationName)}&qty=${encodeURIComponent(String(quantity))}`;
    const res = await fetch(url, { headers: { accept: 'application/json' } });
    if (!res.ok)
        throw new Error(`Dummy vendor ${vendor} failed: ${res.status}`);
...
export async function getBestOffersForMedications(input) {
    ...
    const vendors = ['site-a', 'site-b', 'site-c'];
    // Compute per-vendor totals; include delivery fee once per vendor.
    const totalsByVendor = { 'site-a': 0, 'site-b': 0, 'site-c': 0 };
    const offers = [];
    ...
    for (const vendor of vendors) {
        ...
        if (first)
            totalsByVendor[vendor] += first.deliveryFee;
    }
    const selectedVendor = vendors.reduce((best, vendor) => totalsByVendor[vendor] < totalsByVendor[best] ? vendor : best, vendors[0]);
