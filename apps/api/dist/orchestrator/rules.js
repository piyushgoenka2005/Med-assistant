...
import { getBestOffersForMedications } from './pricing.js';
...
export async function buildCartFromExtraction(input) {
    ...
    const selectedVendor = vendors.reduce((bestVendor, vendor) => perVendorQuote[vendor].total < perVendorQuote[bestVendor].total ? vendor : bestVendor, vendors[0]);
    const selectedOffers = best.offers.filter((o) => o.vendor === selectedVendor);
    const offerByName = new Map(selectedOffers.map((o) => [o.medicationName.toLowerCase(), o]));
    const db = getDb();
    const cartRef = db.collection(collections.carts).doc(input.prescriptionId);
    const items = extraction.medications.map((m) => ({
        name: m.name,
