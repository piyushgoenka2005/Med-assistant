...
export async function prescriptionRoutes(app) {
    app.get('/:id', async (req, reply) => {
        ...
        const [uploadSnap, extractionSnap, cartSnap] = await Promise.all([
            pres?.uploadId ? db.collection(collections.uploads).doc(String(pres.uploadId)).get() : Promise.resolve(null),
    ...
    app.post('/:id/confirm', async (req, reply) => {
        ...
        const medicines = extracted.medications.map((m) => ({
            name: m.name,
        ...
        if (customerId) {
            ...
            if (customerSnap.exists) {
                ...
                const existingRecords = Array.isArray(customer.records) ? customer.records : [];
