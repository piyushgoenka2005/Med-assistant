...
function renderHtml(site, name, qty) {
...
export async function dummySiteRoutes(app) {
        ...
        app.get(`/${site}/orders`, async (req, reply) => {
            ...
            const rows = orders
                ...
                const items = Array.isArray(o.items)
                    ? o.items
                        .map((i) => `${escapeHtml(String(i.name))} x${escapeHtml(String(i.quantity))}`)
                        .join('<br/>')
                    : '';
                return `
            <tr>
